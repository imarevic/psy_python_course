...
def processMouseEvents():
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            sqDistX = (mousePosX - expGlobals["circlePos"][0])**2
            sqDistY = (mousePosY - expGlobals["circlePos"][1])**2
            # check if square root distance smaller than radius
            if math.sqrt(sqDistX + sqDistY) < expGlobals["radius"]:
                expGlobals["response"] = 1
                print("Hit!")
            else:
                expGlobals["response"] = 0
                print("Miss!")
#  END CODE HERE #
