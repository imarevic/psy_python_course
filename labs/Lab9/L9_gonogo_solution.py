...
def prepare_color(stimulus):
    ...
    if stimulus == "nogo":
        # set color red
        settings["stimcolor"] = settings["redColor"]
    else:
        # set color green
        settings["stimcolor"] = settings["greenColor"]


def draw_stimulus(duration):
    """
    draws stimuli to screen fo dration of time.
    arg: duration
    """
    ...
    while settings["response"] != "resp" and (pygame.time.get_ticks()/1000 - t0/1000 < duration):

        # process responses
        process_response_event()
        # draw circle to backbuffer
        pygame.draw.circle(settings["screen"], settings["stimcolor"], settings["circlePos"], settings["radius"])
        # flip to foreground
        pygame.display.flip()

    # record reaction time if resp is there
    if settings["response"] == "resp":
        # get rt
